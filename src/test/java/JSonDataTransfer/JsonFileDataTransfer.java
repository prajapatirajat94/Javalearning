...
public class JsonFileDataTransfer {
	...
	public static void main(String[] args) throws IOException {
		...
		JsonNode node = Ob_MP.readTree(new File("/Users/rajatkumarprajapati/eclipse-workspace/"
				+ "Javalearning/src/test/java/JSonDataTransfer/color.json"));

		// System.out.println(node.path(0).findPath("cars").size());
		JsonNode AddNode = node.path("ClientProfile").findPath("Clients").findPath("Client1").findValue("Address");
